...
def compare(p,q):
    ...
    b.sort(cmp)
    print a , b
    if len(a)==2 and A.index(a[0])+A.index(a[1])==27:
        return p
    if len(b)==2 and A.index(b[0])+A.index(b[1])==27:
        return q
    if len(a)==4 and len(b)!=len(a):
        return p
    if len(b)==4 and len(a)!=len(b):
        return q
    if len(a)==len(b):
        if A.index(a[0])>A.index(b[0]):
            return p
        return q
    return 'ERROR'
